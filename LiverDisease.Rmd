...
# running the model without any tuning parameter
train_knn <- train(Disease ~ ., method = "knn", data = train)
...
Building the ensemble by using knn F1 and random forests with equal weights:

```{r, echo=T}

# creating the ensemble model by using knn F1 and random forests with equal weights
ensemble <- cbind(
                  knn = pred_knn_F1 == "yes",
                  rf = pred_rf == "yes"
                  )
...
# the 2 models in the ensemble are given equal weights
ensemble_preds <- ifelse(rowMeans(ensemble) > 0.5, "yes", "no")
